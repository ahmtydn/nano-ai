...
// Function to populate demo files for new organizations (removed for production)
async function addDemoFiles(ctx, orgId, semester, branch) {
  // Demo files removed - organizations start with empty knowledge nest
  return;
}
export const createOrUpdateOrg = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (!args.org_mail.endsWith("edu.tr")) {
        return { success: false, message: "Only edu.tr domain is allowed" };
      }
      ...
      if (existingUserOrg) {
        ...
        return {
          success: true,
          message: "Organization record updated",
          id: existingUserOrg._id,
        };
      } else {
        ...
        return {
          success: true,
          message: "Organization record created",
          id: userOrgId,
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        message: "Failed to create/update organization record",
      };
    }
  },
});
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (!userOrg || !userOrg.isActive) {
        return {
          success: false,
          message: "User organization record not found",
        };
      }
