...
export const generateReview = mutation({
  ...
  async handler(ctx, args) {
    ...
    if (conceptBreakdown.length < 2) {
      console.log(
        `Generated only ${conceptBreakdown.length} concepts, adding generic concept`
      );
          ...
          mastery_level:
            quizData.score >= 70
              ...
              : quizData.score >= 50
                ...
                : "Needs Improvement",
          suggestion:
            "Keep expanding your knowledge across different areas of this subject.",
        });
      }
    }
...
function analyzeQuizByConceptAreas(quizData) {
  ...
  quizData.questions.forEach((question) => {
    const possibleConcepts = extractConceptsFromQuestion(
  ...
  const identifiedSubtopics = subtopics.filter((subtopic) =>
    incorrectQuestions.some(
      (q) =>
        ...
        (q.explanation &&
          q.explanation.toLowerCase().includes(subtopic.toLowerCase()))
    )
  );
...
function identifyMisconceptions(questions, concept) {
  ...
  const patterns = {
    variables: {
      ...
      misconception:
        "You may be confusing variable scope or declaration types.",
    },
    functions: {
      keywords: ["return", "parameter", "argument", "call", "invoke"],
      misconception:
        "You seem to have difficulty with function parameters or return values.",
    },
    loops: {
      keywords: ["iteration", "index", "break", "continue", "condition"],
      misconception:
        "You might be struggling with loop conditions or iteration control.",
    },
...
function getAdvancedTopics(concept, subject) {
  const advancedTopicsMap = {
    ...
    promises:
      "custom promise implementations and advanced error handling strategies",
...
function generateFeedback(quizData) {
  ...
  const questionsWithAnalysis = quizData.questions.filter(
    (q) => q.analysisData
  );
  const avgConfidence =
    questionsWithAnalysis.length > 0
      ? questionsWithAnalysis.reduce(
          (sum, q) => sum + q.analysisData.confidence,
          0
        ) / questionsWithAnalysis.length
      : 0;
  ...
  if (questionsWithAnalysis.length > 0) {
    if (avgConfidence > 80) {
      feedback +=
        "Your answers showed high confidence levels, indicating good understanding. ";
    } else if (avgConfidence > 60) {
      feedback +=
        "Your answers showed moderate confidence, suggesting some areas need reinforcement. ";
    } else {
      feedback +=
        "Your answers showed lower confidence levels, indicating fundamental concepts need more study. ";
    }
  }
  ...
  if (percentCorrect >= 80) {
    ...
    feedback +=
      "Good job! You have a solid foundation, but there are some areas where you could improve.";
  } else if (percentCorrect >= 40) {
    feedback +=
      "You're making progress, but there are several concepts that need more attention.";
  } else {
    feedback +=
      "This topic seems challenging for you. I recommend revisiting the fundamentals before moving forward.";
  }

  // Add specific insights from incorrect answers
  const incorrectAnswers = quizData.questions.filter(
    (q) => !q.isCorrect && q.analysisData
  );
...
function generateSuggestions(quizData) {
  ...
  let suggestions =
    "Based on your answers, here are some specific areas to focus on:\n\n";
  ...
  incorrectQuestions.forEach((q) => {
    if (q.analysisData && q.analysisData.conceptsIdentified) {
      ...
      const concepts = extractConceptsFromQuestion(
        q.questionText,
        quizData.subject
      );
...
function identifyStrengths(quizData) {
  ...
  if (significantStrengths.length === 0) {
    strengths +=
      "• You answered some questions correctly, but need more practice to develop strong concept mastery.";
  } else {
...
function extractConceptsFromQuestion(
  ...
  if (
    analysisData &&
    analysisData.conceptsIdentified &&
    analysisData.conceptsIdentified.length > 0
  ) {
  ...
  subjectConcepts.forEach((concept) => {
    ...
    if (
      conceptVariations.some((variation) =>
  ...
  if (detectedConcepts.length === 0) {
    ...
    if (words.length > 0) {
      ...
      const importantWord =
        words.find((word) => !commonWords.includes(word)) || words[0];
      detectedConcepts.push(
        importantWord.charAt(0).toUpperCase() + importantWord.slice(1)
      );
    } else {
...
function generateLearningResources(quizData, conceptBreakdown) {
  ...
  conceptBreakdown.forEach((concept) => {
    ...
    const relevantQuestions = quizData.questions.filter((q) => {
      const questionConcepts = extractConceptsFromQuestion(
        q.questionText,
        quizData.subject
      );
    ...
    resources.push({
      ...
      description: `A comprehensive guide that covers ${conceptName} fundamentals with clear explanations and examples${
        keyTerms.length > 0
          ? `, including ${keyTerms
      ...
      type: "Book/Guide",
      difficulty:
        masteryLevel === "Needs Improvement" ? "Beginner" : "Intermediate",
...
function extractMisconceptionTopics(questions, concept) {
  ...
  const defaultTopics = [
    `understanding ${concept} basics`,
    `applying ${concept} correctly`,
  ];
  ...
  questions.forEach((q) => {
    if (q.explanation) {
      ...
      if (
        explanation.includes("common mistake") ||
