...
export default function NanoAILayout({ children, params }) {
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          title: 'NanoAI Chat Session',
          text: 'Check out this NanoAI chat session',
          url: chatLink
        });
      } else {
    ...
    <ProtectedRoute>
      <SidebarContext.Provider value={{ sidebarOpen, setSidebarOpen, isMobile }}>
        <div className={`min-h-screen ${theme === 'dark' ? 'bg-black' : 'bg-gray-50'} transition-colors duration-200`}>
          {/* Navbar is completely hidden in NanoAI */}
          ...
          <div className={`${sidebarOpen ? (isMobile ? 'w-full' : 'w-80') : 'w-16'} ${
          ...
          } border-r flex flex-col transition-all duration-300 fixed left-0 top-0 h-full z-50 shadow-2xl NanoAI-sidebar`}>
            ...
            <div className="flex-1 overflow-y-auto NanoAI-scroll NanoAI-chat-list">
              {sidebarOpen ? (
                sessions === undefined ? (
                  ...
                  <div className="p-3 space-y-2">
                    {filteredSessions.map((session) => (
                      <div
                        key={session._id}
                        className={`group relative p-4 rounded-xl cursor-pointer border transition-all transform hover:scale-[1.02] backdrop-blur-sm hover:shadow-lg NanoAI-chat-item ${
                          pathname.includes(session.sessionId) ? 'NanoAI-chat-item-active' : ''
                        } ${
                            ...
                            <div className="flex items-center justify-between mt-3">
                              ...
                              <div className={`flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200`}>
                                <button
                                  ...
                                  className={`NanoAI-action-btn p-1.5 rounded-md transition-all hover:scale-110 ${
                                    theme === 'dark' 
