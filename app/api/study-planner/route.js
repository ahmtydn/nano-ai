...
export async function POST(request) {
  try {
    ...
    if (
      mode === "manual" &&
      (!manualInput || !manualInput.subjects || !manualInput.topics)
    ) {
      return NextResponse.json(
        {
    ...
    if (mode === "ai-review") {
      ...
      if (!aiReviews || aiReviews.length === 0) {
        return NextResponse.json(
          {
            error:
              "No quiz reviews found. Please take some quizzes first to generate a personalized study plan.",
            success: false,
          },
          { status: 400 }
        );
...
Focus on practical, actionable tasks that build upon each other. Ensure the difficulty progresses logically and aligns with the specified level: ${manualInput.difficulty}.`;
  ...
  try {
    ...
    const processedNotes = parsedPlan.notes.map((note, index) => {
      const dayRange = note.day_range
        ? note.day_range.split("-")
        : [index + 1, index + 2];
    ...
    const plannerId = await convex.mutation(
      api.studyPlanner.createManualPlanner,
      {
        username,
        type,
    ...
    throw new Error(
      "Failed to generate study plan with AI: " + aiError.message
    );
  }
}
...
export async function PATCH(request) {
  try {
    ...
    const result = await convex.mutation(
      api.studyPlanner.toggleNoteCompletion,
      {
        noteId,
        isCompleted,
      }
...
export async function DELETE(request) {
  try {
    ...
    if (action === "cleanup") {
      ...
      const result = await convex.mutation(
        api.studyPlanner.cleanupExpiredPlanners,
        {}
      );
