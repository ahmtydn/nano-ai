...
function processExplanationWithStyle(topic, style, originalTranscript) {
  ...
  const styleInstructions = {
    "formal academic":
      "Provide a formal, academic explanation with proper terminology, structured format, and scholarly depth. Use precise language and comprehensive coverage.",
    "story format":
      "Tell this as an engaging story with characters, plot, and narrative flow. Make it memorable and entertaining while being educational.",
    "movie style":
      "Explain this dramatically like a movie scene with vivid descriptions, analogies, and cinematic language. Make it exciting and visual.",
    "casual chat":
      "Explain this like you're talking to a close friend over coffee - relaxed, friendly, using everyday language and relatable examples.",
    "step-by-step":
      "Break this down into clear, numbered steps that are easy to follow. Make each step actionable and build logically.",
    "analogy mode":
      "Use simple analogies and comparisons to everyday things the student already knows. Make complex concepts relatable.",
  };
...
async function handleSingleAgentVoice(ws, data, sessionContext) {
  ...
  try {
    ...
    if (
      isExplanationRequest(transcript) &&
      !sessionContext.awaitingStyleChoice
    ) {
      // Extract topic from the explanation request
      const topic = transcript
        .replace(
          /^(can you |please |could you |)?(explain|tell me about|what is|what are|how does|how do)/i,
          ""
        )
        .trim();
    ...
    const conversationalPrompt =
      voiceMode === "conversational"
        ? `You are NanoAI, a friendly AI learning companion in LIVE VOICE mode. Respond like you're having a natural conversation with a student.
...
Respond naturally and briefly:`
        : `You are NanoAI, an AI learning companion. Provide a detailed but voice-friendly response to the student's question. Keep it conversational but comprehensive.
...
async function handleMultiAgentVoice(ws, data, sessionContext) {
  ...
  try {
    ...
    const result = await sessionContext.agentSystem.processInput(
      transcript,
      context
    );
    ...
    if (sessionContext.conversationBuffer.length > 10) {
      sessionContext.conversationBuffer =
        sessionContext.conversationBuffer.slice(-10);
    }
    ...
    if (result.type === "info_request") {
      ...
      if (result.suggestedQuestions && result.suggestedQuestions.length > 0) {
        ...
        if (voiceMode === "conversational") {
          ...
          responseText +=
            " Here are some suggestions: " + suggestions.join(", or ");
        }
      }
      ...
      responseText =
        voiceMode === "conversational"
          ...
          : "Great! I've created a quiz for you. You can access it in the NanoAI interface.";
    }
