...
async function processFile(file) {
  try {
    ...
    console.log(
      `[FILE PROCESSING] Starting to process file: ${file.name} (${(file.size / 1024).toFixed(2)} KB)`
    );
    ...
    console.log(
      `[FILE PROCESSING] File buffer created successfully: ${fileSize} KB`
    );
    ...
    if (fileName.endsWith(".pdf")) {
      ...
      try {
        ...
        if (pdfData.text && pdfData.text.trim().length > 0) {
          ...
          const wordCount = pdfData.text
            .split(/\s+/)
            .filter((word) => word.length > 0).length;
          const pageCount = pdfData.numpages;

          console.log(
            `[FILE PROCESSING] PDF text extraction successful: ${wordCount} words, ${pageCount} pages`
          );
...
${sanitizedText}`;
          ...
          console.log(
            `[FILE PROCESSING] PDF text extraction returned empty content`
          );
...
${displayContent}${truncated ? "\n\n[Content truncated - showing first 8,000 characters]" : ""}`;
        ...
        console.error(
          `[FILE PROCESSING] Text file processing error:`,
          textError
        );
...
Current status: File structure analyzed and ready for processing.`;
      ...
      console.log(
        `[FILE PROCESSING] Processing PowerPoint presentation: ${file.name}`
      );
...
Current status: Presentation structure analyzed and ready for processing.`;
      ...
      console.log(
        `[FILE PROCESSING] Processing Excel spreadsheet: ${file.name}`
      );
...
This file type is supported for upload but may require specific processing based on its content.`;
    ...
    console.log(
      `[FILE PROCESSING] File processing completed successfully: ${file.name}`
    );
...
async function analyzeImage(imageData, userMessage, model) {
  try {
    const prompt =
      userMessage ||
      "Please analyze this image in detail. Describe what you see, including objects, people, activities, colors, composition, and any text visible in the image.";
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        ...
        console.error(
          "JSON parsing error detected, sanitizing content and retrying..."
        );
...
export async function POST(request) {
  try {
    ...
    if (isImageAnalysis && fileProcessingResult.data) {
      ...
      try {
        console.log(
          `[IMAGE ANALYSIS] Starting image analysis for: ${fileProcessingResult.data.fileName}`
        );
...
`;
        ...
        fileProcessingResult.type === "error"
      ) {
    ...
    console.log(
      `[AI GENERATION] Starting content generation with model: ${modelName}`
    );
    console.log(
      `[AI GENERATION] Prompt length: ${fullPrompt.length} characters`
    );
    ...
    if (
      ...
      errorMessage =
        "Content contains invalid characters that cannot be processed. Please try rephrasing your message or uploading a different file.";
      statusCode = 400;
      ...
      errorMessage =
        "Request timeout. Please try again with a shorter message.";
      statusCode = 408;
      retryable = true;
    } else if (
      ...
      errorMessage =
        "Model not available. Please try again with a different configuration.";
      statusCode = 400;
    }
    ...
    return NextResponse.json(
      {
        ...
        success: false,
        retryable: retryable,
        errorType: error.message?.includes("invalid json")
          ? "JSON_PARSE_ERROR"
          : "UNKNOWN",
      },
      { status: statusCode }
