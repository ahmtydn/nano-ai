...
function buildNanoAIContext(messages) {
  if (!messages || messages.length === 0) {
    return `You are NanoAI, an AI learning companion designed to help students learn effectively. You should:
...
Start the conversation by greeting the student warmly and asking how you can help with their studies today.\n\n`;
  ...
  let context = `You are NanoAI, an AI learning companion. Here's our conversation history:\n\n`;
  ...
  context += `Continue the conversation as NanoAI, maintaining your helpful and educational tone. Use emojis and formatting to make your response engaging and easy to understand.\n\nStudent's new message:\n`;
...
export async function POST(request) {
  try {
    ...
    if (isImageAnalysis && fileProcessingResult.data) {
      const userMessage =
        message ||
        "Please analyze this image and help me understand what I'm looking at.";
      ...
      try {
        ...
        const imageAnalysisResult = await analyzeImage(
          ...
          `As NanoAI, analyze this image and help the student understand it. ${userMessage}`,
          model
        );
    ...
    const contextPrompt = buildNanoAIContext(existingMessages);
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        if (retryCount >= MAX_RETRIES) {
          throw new Error(
            `Failed to generate response after ${MAX_RETRIES} attempts: ${error.message}`
          );
        }
    ...
    console.error("NanoAI API Error:", error);
    return NextResponse.json(
      {
