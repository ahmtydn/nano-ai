...
`;
            ...
            if (
              quizError.message.includes(
                "Failed to generate valid quiz questions"
              )
            ) {
              errorMessage =
                "I had trouble generating questions for this topic. Could you try being more specific about what you'd like to be quizzed on?";
            } else if (
              quizError.message.includes("Convex") ||
              quizError.message.includes("database")
            ) {
              errorMessage =
                "There was a temporary issue saving your quiz. Please try again in a moment.";
            } else if (
              quizError.message.includes("API key") ||
              quizError.message.includes("authentication")
            ) {
              errorMessage =
...
**Error Details**: ${quizError.message}`;
            ...
            responseData = {
              ...
              errorType: quizError.message.includes("Convex")
                ? "database"
                : quizError.message.includes("API")
                  ? "api"
                  : "generation",
              systemStatus: agentSystem.getSystemStatus(),
            };
          ...
          if (
            result.data?.type === "quiz_selection" ||
            result.data?.type === "initial_review" ||
            (result.data?.type === "general_tutor_response" &&
              result.message.includes("fetch your recent quiz"))
          ) {
            try {
              // Fetch user's recent quizzes (last 3)
              const userQuizzes = await convex.query("quiz:getUserQuizzes", {
                username,
              });
              const recentQuizzes = userQuizzes
                ...
                .sort(
                  (a, b) =>
                    (b.attemptedAt || b.createdAt) -
                    (a.attemptedAt || a.createdAt)
                )
                .slice(0, 3);
...
${recentQuizzes
  .map((quiz, index) => {
    const date = new Date(
      quiz.attemptedAt || quiz.createdAt
    ).toLocaleDateString();
...
**Error**: ${error.message}`;
          ...
          // Handle quiz analysis - when user selects a specific quiz
            ...
            result.data?.type === "quiz_analysis" &&
            result.data?.selectedQuiz
          ) {
            try {
              ...
              let aiReview = await convex.query("aiReview:getReviewByQuizId", {
              ...
              if (!aiReview) {
                ...
                aiReview = await convex.query("aiReview:getReviewByQuizId", {
                  quizId: selectedQuizId,
                });
              }
              ...
              if (aiReview) {
                // Get quiz details
                const quiz = await convex.query("quiz:getQuizById", {
                  quizId: selectedQuizId,
                });
...
${Object.entries(aiReview.learning_resources || {})
  .map(
    ([concept, data]) =>
      `\n**${concept}:**\n${data.resources
        .slice(0, 2)
          ...
          // Handle when user selects a quiz by number or makes a selection
            ...
            if (enhancedMessage.startsWith("ANALYZE_QUIZ_ID:")) {
              ...
              if (numberMatch) {
                ...
                enhancedMessage.toLowerCase().includes("first") ||
                enhancedMessage.toLowerCase().includes("1")
              ) {
                quizNumber = "1";
              } else if (
                enhancedMessage.toLowerCase().includes("second") ||
                enhancedMessage.toLowerCase().includes("2")
              ) {
                quizNumber = "2";
              } else if (
                enhancedMessage.toLowerCase().includes("third") ||
                enhancedMessage.toLowerCase().includes("3")
              ) {
                quizNumber = "3";
              }
              ...
              if (quizNumber) {
                ...
                try {
                  const userQuizzes = await convex.query(
                    "quiz:getUserQuizzes",
                    { username }
                  );
                  const recentQuizzes = userQuizzes
                    .filter(
                      (quiz) => quiz.completed && quiz.score !== undefined
                    )
                    .sort(
                      (a, b) =>
                        (b.attemptedAt || b.createdAt) -
            ...
            if (selectedQuizId) {
              ...
              try {
                // Check if AI review exists, if not generate it
                let aiReview = await convex.query(
                  "aiReview:getReviewByQuizId",
                  { quizId: selectedQuizId }
                );
...
**Error**: ${agentError.message}`;
      ...
      // This is essentially the original NanoAI logic
      ...
      let context = `You are NanoAI, an AI learning companion. Help the student with their request.\n\nStudent: ${enhancedMessage}`;

      if (existingMessages && existingMessages.length > 0) {
        const recentMessages = existingMessages.slice(-10);
        const historyContext = recentMessages
          .map((msg) => {
            const role = msg.role === "user" ? "Student" : "NanoAI";
            return `${role}: ${msg.content}`;
          })
          .join("\n");

        context = `You are NanoAI, an AI learning companion. Here's our conversation history:\n\n${historyContext}\n\nContinue the conversation as NanoAI.\n\nStudent's new message: ${enhancedMessage}`;
      }
    ...
    console.error("Multi-Agent NanoAI API Error:", error);
    return NextResponse.json(
      {
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json({
      message: "Multi-Agent NanoAI API",
