/**
 * Multi-Agent AI System for NanoAI
 *
...
class OrchestratorAgent extends BaseAgent {
  constructor() {
    super(
      ...
      "Orchestrator",
      "Central AI controller that routes requests to appropriate agents"
    );
  ...
  async process(input, context = {}) {
    ...
    try {
      ...
      this.addToConversation(
        "orchestrator",
        `Routing to ${routingDecision.targetAgent}: ${routingDecision.reasoning}`,
        {
  ...
  async analyzeAndRoute(input, context) {
    const prompt = `You are the Orchestrator Agent in a multi-agent AI system for NanoAI. Your job is to analyze user requests and route them to the most appropriate specialized agent.
...
- "Analyze my learning progress" → TUTOR agent`;
    ...
    try {
      ...
      return {
        targetAgent: input.toLowerCase().includes("quiz")
          ? "QUIZ"
...
class QuizAgent extends BaseAgent {
  constructor() {
    super(
      AGENT_TYPES.QUIZ,
      "Quiz Agent",
      "Specialized agent for creating and managing quizzes"
    );
    this.model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
      generationConfig: {
...
🚀 **Ready to start your quiz?** Click the button below!`,
      ...
      this.setState(AGENT_STATES.IDLE);
      this.addToConversation("quiz_agent", `Error: ${error.message}`, {
        error: true,
      });
      throw error;
    }
...
Just tell me something like "Quiz me on Biology" or "Create a math quiz on algebra" and I'll take care of the rest!`,
  ...
  generateNextSuggestions(extractedInfo) {
    ...
    suggestions.push({
      ...
      description:
        "Analyze your quiz results and get personalized improvement suggestions",
      action: "tutor_review",
  ...
  getRelatedTopics(subject, currentTopic) {
    ...
    return topics
      .filter((topic) => topic.toLowerCase() !== currentTopic?.toLowerCase())
      .slice(0, 2);
  }
}
...
class GeneralStudyAgent extends BaseAgent {
  constructor() {
    super(
      AGENT_TYPES.GENERAL,
      "Study Assistant",
      "General purpose study helper for explanations and problem solving"
    );
    this.model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
      generationConfig: {
  ...
  async generateStudyResponse(input, context) {
    ...
    const prompt = `You are NanoAI's Study Assistant Agent, specialized in helping students learn effectively. You should:
...
${conversationHistory.map((msg) => `${msg.role === "user" ? "Student" : "NanoAI"}: ${msg.content}`).join("\n")}
...
class TutorAgent extends BaseAgent {
  constructor() {
    super(
      AGENT_TYPES.TUTOR,
      "Performance Tutor",
      "AI tutor specialized in analyzing quiz performance and providing personalized improvement suggestions"
    );
    this.model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
      generationConfig: {
  ...
  async process(input, context = {}) {
    ...
    try {
      ...
      switch (requestType.type) {
        ...
        case "quiz_selection":
          response = await this.handleQuizSelection(
            requestType.selectedQuiz,
            context
          );
          responseData = {
            type: "quiz_analysis",
  ...
  "reasoning": "brief explanation"
    ...
    try {
      ...
      if (
        ...
        (lowerInput.includes("quiz") || lowerInput.includes("performance"))
      ) {
        return {
          type: "initial_review",
          reasoning: "User asking for recent quiz performance",
        };
      } else if (
        lowerInput.includes("performance") ||
        lowerInput.includes("how did i") ||
        lowerInput.includes("my quiz")
      ) {
        return {
          type: "initial_review",
          reasoning: "User asking for quiz performance review",
        };
      } else if (
        (lowerInput.includes("quiz") &&
          (lowerInput.includes("1") || lowerInput.includes("first"))) ||
        (lowerInput.includes("analyze") && lowerInput.includes("1"))
      ) {
        return {
          type: "quiz_selection",
          selectedQuiz: "1",
          reasoning: "User selecting quiz 1",
        };
      } else if (
        lowerInput.includes("quiz") &&
        (lowerInput.includes("2") || lowerInput.includes("second"))
      ) {
        return {
          type: "quiz_selection",
          selectedQuiz: "2",
          reasoning: "User selecting quiz 2",
        };
      } else if (
        lowerInput.includes("quiz") &&
        (lowerInput.includes("3") || lowerInput.includes("third"))
      ) {
        return {
          type: "quiz_selection",
          selectedQuiz: "3",
          reasoning: "User selecting quiz 3",
        };
      } else if (
        lowerInput.includes("select") ||
        lowerInput.includes("choose") ||
        lowerInput.includes("analyze")
      ) {
        return {
          type: "quiz_selection",
          reasoning: "User seems to be selecting a quiz",
        };
      } else if (lowerInput.includes("detail") || lowerInput.includes("more")) {
        return {
          type: "detailed_analysis",
          reasoning: "User wants more details",
        };
      } else if (
        ...
        return {
          type: "further_help",
          reasoning: "User wants additional help",
        };
      } else {
        return {
          type: "initial_review",
          reasoning: "Default to initial review",
        };
      }
    }
  }
...
export class AgentSystem {
  ...
  async processInput(input, context = {}) {
    try {
      ...
      const routing = await this.agents[AGENT_TYPES.ORCHESTRATOR].process(
        input,
        context
      );
      ...
      this.logInteraction(
        "system",
        `Orchestrator routed to ${routing.targetAgent}: ${routing.reasoning}`
      );
      ...
      const result = await this.agents[targetAgent].process(
        input,
        enhancedContext
      );
      ...
      this.logInteraction(
        "system",
        `${targetAgent} agent completed processing`
      );
